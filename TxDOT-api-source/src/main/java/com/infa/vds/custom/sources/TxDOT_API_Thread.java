 ...
 *
 */
class TxDOT_API_Thread implements Runnable {

	static final short Number_Connections = 1;
	static final short Number_Disconnections = 2;

	Logger _logger = LoggerFactory.getLogger(TxDOT_API_source.class);
	...
	TxDOT_API_Thread(TxDOT_API_Shared sourceConfig) {
		config = sourceConfig;
	}

	public void run() {

		TxDOT_API_source src = config.sourceObj;
		...
		_logger.info("Exiting TxDOT_API_Thread.run()");
	}
}
