...
public class HttpXSource implements VDSSource, VDSPluginStatistics {

	// Note: Field name should be same as in vdsplugin.xml
	public static final String URL_TEXT_FIELD_NAME = "URL";
	...
	private Logger _logger = LoggerFactory.getLogger(HttpXSource.class);
	HttpXSource_Shared config;
	HttpX_Thread httpx_reader;
	...
	public void open(VDSConfiguration vdsConfiguration) throws Exception {
		...
		config = new HttpXSource_Shared();

		// parse all the configuration
		parseConfig(vdsConfiguration);
		config.eventQueue = new LinkedBlockingQueue<>(HttpXSource_Shared.EVENT_QUEUE_SIZE);
		...
		httpx_reader = new HttpX_Thread(config);
	...
	public void parseConfig(VDSConfiguration vdsConfiguration) throws Exception {
		...
		config.strURL = vdsConfiguration.getString(URL_TEXT_FIELD_NAME);
		config.url = new URL(config.strURL);
		config.host = config.url.getHost();
		config.path = config.url.getPath();
		config.query = config.url.getQuery();
		config.port = config.url.getPort();

		String info = String.format("Host = %s:%d\n; Path = %s;\nQuery = %s;\n", config.host, config.port, config.path,
				config.query);
		_logger.info(info);
	}
