...
//	private static String pcid = "";
	private static boolean useStdin;
	...
	public static void main(String[] args)
	{
		...
		longopts[10] = new LongOpt("stdin", LongOpt.NO_ARGUMENT, null, OPTION_STDIN);
		...
		boolean done = false;
		long count;
		for (count = 0; (useStdin || count < msgs) && !done; )
		{
		    ...
		    for(int ivlcount = 0; (useStdin || ivlcount < msgs_per_ivl); ivlcount++) 
			{
				...
				try
				{
			    	...
			    	if (useStdin) {
			    		String strLine = null;
			    		try {
			    			strLine = stdinReader.readLine();
			    		} catch (IOException ie) {
			    			System.err.println("Error reading from stdin!");
							ie.printStackTrace(System.err);
							done = true;	// Force outer loop exit
							break;
						}
						if (strLine == null) {
							done = true;
