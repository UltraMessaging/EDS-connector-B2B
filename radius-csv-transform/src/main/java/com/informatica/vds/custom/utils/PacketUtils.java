...
public class PacketUtils {
	...
	public static String inetBytes2Str(byte [] bytes) {
		String strInet = "";
		for (int i = 0; i < 4; i++) {
			if (i > 0) {
				strInet += ".";
	...
	public RadiusAvp[] readRadAvps(ByteBuffer bb, int avpLength) {
		...
		while (radRemain >= 2) {
			...
			radRemain -= 2;
			if (working.length <= 2 || radRemain < (working.length - 2)) {
	...
	/**
	 * 
	 * @param out - if non-null, points to a PrintStream to print the output to
	 * @param bb - input ByteBuffer
	 * @param incl_len - Length to print from buffer
	 * @param printOffset - If true, separate output into lines, each with 16 bytes and preceded by offset in hex
	 * @return A string representing the input bytes as hex digits
	 */
	public static String hexprint(PrintStream out, ByteBuffer bb, int incl_len, boolean printOffset) {
	...
	/**
	 * Converts bytes to ascii, and Inserts quote/escape chars as needed
	 * @param data
	 * @return String value surrounded by quotes, quotes are escaped in place as needed
	 */
