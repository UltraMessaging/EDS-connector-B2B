...
public class WS_API_source implements VDSSource, VDSPluginStatistics {
	...
	private Logger _logger = LoggerFactory.getLogger(WS_API_source.class);
	WS_API_Shared config;
	WS_API_Thread httpx_reader;
	...
	public void open(VDSConfiguration vdsConfiguration) throws Exception {
		...
		config = new WS_API_Shared();

		// parse all the configuration
		parseConfig(vdsConfiguration);
		config.eventQueue = new LinkedBlockingQueue<>(WS_API_Shared.EVENT_QUEUE_SIZE);
		...
		httpx_reader = new WS_API_Thread(config);
