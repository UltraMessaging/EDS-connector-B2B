package com.informatica.messaging.vds.sources.camel.feed;
...
/**
 * Camel Component for feed sources like RSS and Atom
 * @author hpurohit
 *
 */
public class CamelFeedSource implements VDSSource, VDSCamelConstants {

    /*** 
     * Feed plugin properties 
     * 
     ***/
    private static final String FEED_TYPE = "feedType";
    private static final String ATOM = "atom";
    private static final String RSS = "rss";
    private static final String ATOM_FEED_URL = "atomFeedUrl";
    private static final String RSS_FEED_URL = "rssFeedUrl";
    private static final String USER_PASSWORD = "userPassword";
    private static final String USER_NAME = "userName";
    private static final String USE_BASIC_AUTH_FOR_HTTP = "useBasicAuthForHttp";
    private static final String CONSUMER_DELAY = "consumerDelay";
    ...
    private static final Logger _logger = LoggerFactory.getLogger(CamelFeedSource.class);
    ...
    public void close() throws IOException {
        ...
        _logger.info("Closing Camel Feed Source...");
        ...
        try {
            ...
            _logger.error("Exception while closing Camel Feed Source: {}", e.getMessage(), e);
        }
    ...
    public void open(VDSConfiguration vdsConfiguration) throws Exception {
        ...
        _logger.info("Opening Camel Feed Source for reading...");
        ...
        Thread camelTask = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    _logger.error("Exception while opening Camel Feed Source: {}", e.getMessage(), e);
                }
    ...
    public void read(VDSEventList vdsEventList) throws Exception {
        ...
        _logger.debug("Reading from Camel Feed Source...");
    ...
    public void parseConfig(VDSConfiguration vdsConfiguration) throws Exception {
         ...
         */
        feedType = vdsConfiguration.getString(FEED_TYPE);
        boolean isSplitEntries = vdsConfiguration.getBoolean(SPLIT_ENTRIES);
        long consumerDelay = (long) vdsConfiguration.getDouble(CONSUMER_DELAY);
        boolean useBasicAuthForHttp = vdsConfiguration.getBoolean(USE_BASIC_AUTH_FOR_HTTP);
        ...
        if (isSplitEntries) {
            options.append("splitEntries=true");
        } else {
            options.append("splitEntries=false");
        }

        options = options.append("&consumer.delay=" + consumerDelay);

        if (useBasicAuthForHttp) {
            options = options.append("&username=" + vdsConfiguration.getString(USER_NAME));
            options = options.append("&password=" + vdsConfiguration.getString(USER_PASSWORD));
        }
        ...
        if (feedType.equalsIgnoreCase(RSS)) {
            camelFromURI = "rss:" + vdsConfiguration.getString(RSS_FEED_URL) + options.toString();
        } else if (feedType.equalsIgnoreCase(ATOM)) {
            camelFromURI = "atom:" + vdsConfiguration.getString(ATOM_FEED_URL) + options.toString();
        }
    ...
    class VDSCamelRouteBuilder extends RouteBuilder {
        ...
        public void configure() throws Exception {
            ...
            /** Marshal **/
            if (feedType.equalsIgnoreCase(RSS)) {
                marshalProcess = routeDefinition.marshal().rss().convertBodyTo(String.class, CHARSET_UTF_8);
            } else {
            ...
            if (camelRouteDefnConfig.getSplitType().equalsIgnoreCase(SPLIT_TOKEN)) {
                ...
                if (filterAndSplitExprNode == null) {
                    ...
                    filterAndSplitExprNode = filterAndSplitExprNode.split()
                            .tokenizeXML(camelRouteDefnConfig.getSplitXmlTag()).streaming()
                            .convertBodyTo(String.class);
                }
            }
